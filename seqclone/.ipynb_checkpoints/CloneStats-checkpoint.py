...
def preprocessWScanpy(adata, datatype, highly_variable, n_highly_variable, remove_immune_receptors, normalize, filterCells):
    if remove_immune_receptors == True:
        # TODO: Supply this as a metadata file or somehow incorporate this information into the adata.var 
    ...
    sc.pp.filter_genes(adata, min_cells=380, inplace = True)
...
def plotEgHists(df, adata_obs, num_shuffles, gene, label):
    ...
    plt.hist(data_shuffled, color = 'peru', alpha = 0.5)
    plt.hist(data_shuffled, color = 'peru', histtype='step')
    plt.hist(data, color = 'midnightblue', alpha = 0.5)
    plt.hist(data, color = 'midnightblue', histtype='step')
...
def main():
    ...
    # Run the Test on genes specified by parameters
    df_tests = permutationTest(df, 
                                        adata.obs,
